...
# Receives MJPEG streams from Raspberry Pi
...
gst-launch-1.0 -q udpsrc port=5000 ! \
  ...
  jpegdec ! \
  videoconvert ! \
  "video/x-raw,format=BGR,width=192,height=192" ! \
  fdsink | $PYTHON_CMD src/inference_pipe.py --title "Camera 1 (Inference)" --model "$MODEL_PATH" --eye 0 --width 192 --height 192 --data_dir "$SESSION_DIR" &

echo "Starting Inference Stream for Camera 2 (Port 5001)..."
gst-launch-1.0 -q udpsrc port=5001 ! \
  "application/x-rtp,encoding-name=JPEG,payload=26" ! \
  rtpjpegdepay ! \
  jpegdec ! \
  videoconvert ! \
  "video/x-raw,format=BGR,width=192,height=192" ! \
  fdsink | $PYTHON_CMD src/inference_pipe.py --title "Camera 2 (Inference)" --model "$MODEL_PATH" --eye 1 --width 192 --height 192 --data_dir "$SESSION_DIR" &
